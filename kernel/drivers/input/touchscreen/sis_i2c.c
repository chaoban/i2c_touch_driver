...
#endif
//		printk(KERN_INFO "chaoban test: ret = %d\n", ret);
...
//	printk(KERN_INFO "chaoban test: sis_ts_work_func\n");
	...
	if (ret > 2)
	{   
		printk(KERN_INFO "chaoban test: Buf_Data = %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x \r\n",
...
static int sis_ts_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	if (ret < 0)
		printk(KERN_ERR "sis_ts_suspend: i2c_smbus_write_byte_data failed\n");
	...
	if (ts->power) {
		ret = ts->power(0);
		if (ret < 0)
			printk(KERN_ERR "sis_ts_resume power off failed\n");
	}
	return 0;
}
